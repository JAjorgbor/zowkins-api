...
3. **HTTP end-to-end against a throwaway database.** `.env` points at the **real Atlas cluster**, so never run write tests against it.
   ...
   - Customers need a verified email to get tokens. Either run sign-up → capture the code from the stubbed `portalVerifyEmailOtp` call (`args.otp`) → `verify-email`, or create the user directly with `isEmailVerified: true` and mint a token with `tokenService.generateAuthTokens`.
